...
let test_eval_nest_expr1 () =
  let env = Environment.ext (Environment.ext (Environment.emptyEnv()) "x" (IntVal 1)) "x" (IntVal 2) in
  let expr = Var "x" in
  let result = Eval.eval expr env in
  match result with
...
let test_eval_nest_expr2 () =
  ...
  let expr = Let ("x", IntLit 1, Let ("y", Plus (Var "x", IntLit 1), Plus (Var "x", Var "y"))) in
  let result = Eval.eval expr env in
  match result with
...
let test_eval_unbundling_expr () =
  let env = Environment.emptyEnv() in
  let expr = Let ("x", IntLit 1, Plus (Let ("x", IntLit 2, Plus (Var "x", IntLit 1)), Times (Var "x", IntLit 2))) in
  let result = Eval.eval expr env in
  match result with
