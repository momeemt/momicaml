...
let rec type_to_string = function
  | TInt -> "int"
  | TBool -> "bool"
  | TArrow (t1, t2) -> Printf.sprintf "(%s -> %s)" (type_to_string t1) (type_to_string t2)
...
let test_typecheck_env4 () =
  ...
  let env =
    Environment.ext (Environment.emptyEnv()) "x" TBool
  in
  match tcheck env expr with
  | Error e -> Alcotest.(check bool) e true true
  | _ -> Alcotest.fail "typecheck failed"
  ...
  let expr =
    App
      ( Fun ("x", If (BoolLit true, Var "x", IntLit 100)),
        If (BoolLit true, Var "y", IntLit 200) ) in
  let env = Environment.ext (Environment.ext (Environment.emptyEnv()) "x" TInt) "y" TInt in
  match tcheck env expr with
              ...
              ( Var "f",
                App
                  ( Var "f",
                    App
                      ( Var "f",
                        Plus (Var "x", IntLit 10) ) ) ) ) ) in
  let env = Environment.ext (Environment.ext (Environment.emptyEnv()) "x" TInt) "f" (TArrow (TInt, TInt)) in
  match tcheck env expr with
  | Ok (TArrow (TArrow (TInt, TInt), TArrow (TInt, TInt))) -> Alcotest.(check bool) "typecheck_fn4" true true
  | _ -> Alcotest.fail "typecheck failed"
                ...
                App
                  ( Var "f",
                    App
                      ( Var "g",
                        Var "x" ) ) ) ) ) in
  let env = Environment.ext (Environment.ext (Environment.ext (Environment.emptyEnv()) "x" TInt) "g" (TArrow (TInt, TInt))) "f" (TArrow (TInt, TInt)) in
  match tcheck env expr with
  | Ok (TArrow (TArrow (TInt, TInt), TArrow(TArrow (TInt, TInt), TArrow (TInt, TInt)))) -> Alcotest.(check bool) "typecheck_fn5" true true
  | _ -> Alcotest.fail "typecheck failed"
      ...
      ( "tcheck",
        [
          ...
          Alcotest.test_case "test_typecheck_eq_failed" `Quick
            test_typecheck_failed;
          Alcotest.test_case "test_typecheck_env1" `Quick
            test_typecheck_env1;
          Alcotest.test_case "test_typecheck_env2" `Quick
            test_typecheck_env2;
          Alcotest.test_case "test_typecheck_env3" `Quick
            test_typecheck_env3;
          Alcotest.test_case "test_typecheck_env4" `Quick
            test_typecheck_env4;
          Alcotest.test_case "test_typecheck_env5" `Quick
            test_typecheck_env5;
          Alcotest.test_case "test_typecheck_fn1" `Quick
            test_typecheck_fn1;
          Alcotest.test_case "test_typecheck_fn2" `Quick
            test_typecheck_fn2;
          Alcotest.test_case "test_typecheck_fn3" `Quick
            test_typecheck_fn3;
          Alcotest.test_case "test_typecheck_fn4" `Quick
            test_typecheck_fn4;
          Alcotest.test_case "test_typecheck_fn5" `Quick
            test_typecheck_fn5;
        ] );
    ]
