...
let test_compiler_let2 () =
  ...
  let stack =
    match stack_res with Ok stack -> stack | _ -> failwith "Failed to compile"
  in
  let result =
    match List.hd stack with
    | CAM.CAM_IntCal i -> i
...
let test_compiler_if1 () =
  let stack_res =
    Compiler.compile "if true then 1 else 2" (Environment.emptyEnv ())
  in
  let stack =
    match stack_res with Ok stack -> stack | _ -> failwith "Failed to compile"
  in
...
let test_compiler_if2 () =
  let stack_res =
    Compiler.compile "if false then 1 else 2" (Environment.emptyEnv ())
  in
  let stack =
    match stack_res with Ok stack -> stack | _ -> failwith "Failed to compile"
  in
...
let test_compiler_if_not_pass_typecheck1 () =
  let stack_res =
    Compiler.compile "if 1 then 1 else 2" (Environment.emptyEnv ())
  in
  match stack_res with
