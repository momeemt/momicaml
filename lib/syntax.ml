...
let rec string_of_match lst =
  match lst with
  | [] -> ""
  | (e1, e2) :: [] -> string_of_expr e1 ^ " -> " ^ string_of_expr e2
  | (e1, e2) :: t ->
      string_of_expr e1 ^ " -> " ^ string_of_expr e2 ^ " | " ^ string_of_match t
...
and string_of_expr expr =
  ...
  | LetRec (f, x, e1, e2) ->
      "let rec " ^ f ^ " " ^ x ^ " = " ^ string_of_expr e1 ^ " in "
      ^ string_of_expr e2
  ...
  | Empty -> "[]"
  | Match (e, lst) ->
      "match " ^ string_of_expr e ^ " with " ^ string_of_match lst
