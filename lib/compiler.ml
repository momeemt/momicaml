...
module Compiler = struct
  ...
  let parse str tenv =
    ...
    let ast_tchecked = Typecheck.tcheck tenv ast in
    match ast_tchecked with Ok _ -> ok ast | Error e -> error e

  let rec codegen exp venv =
    match exp with
    | Ok (Var x) -> ok [ CAM.CAM_Access (position x venv) ]
    | Ok (Fun (x, e)) -> (
        let res = codegen (ok e) (x :: "_" :: venv) in
        match res with
        | Ok code -> ok (CAM.CAM_Closure code :: [ CAM.CAM_Return ])
        | Error e -> Error e)
    | Ok (App (e1, e2)) -> (
        let res1 = codegen (ok e2) venv in
        match res1 with
        | Ok code1 -> (
            let res2 = codegen (ok e1) venv in
            match res2 with
            | Ok code2 -> ok (code1 @ code2 @ [ CAM.CAM_Apply ])
            | Error e -> Error e)
        | Error e -> Error e)
    | Ok (Let (x, e1, e2)) -> (
        let res1 = codegen (ok e1) venv in
        match res1 with
        | Ok code1 -> (
            let res2 = codegen (ok e2) (x :: venv) in
            match res2 with
            | Ok code2 ->
        ...
        | Error e -> Error e)
    | Ok (LetRec (f, x, e1, e2)) -> (
        let res1 = codegen (ok e1) (x :: f :: venv) in
        match res1 with
        | Ok code1 -> (
            let res2 = codegen (ok e2) (f :: venv) in
            match res2 with
            | Ok code2 ->
                ok
    ...
    | Ok (If (e1, e2, e3)) -> (
        ...
        match res1 with
        | Ok code1 -> (
            let res2 = codegen (ok e2) venv in
            match res2 with
            | Ok code2 -> (
                let res3 = codegen (ok e3) venv in
                match res3 with
