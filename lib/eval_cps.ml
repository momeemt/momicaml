...
module EvalCPS = struct
  let rec eval expr env cont =
    ...
    match expr with
      ...
      | Let (x, e1, e2) -> (
        eval e1 env (fun v1 ->
        let env1 = Environment.ext env x v1
        in eval e2 env1 cont)
      )
      | LetRec (f, x, e1, e2) -> (
        let env1 = Environment.ext env f (RecFunVal (f, x, e1, env))
        in eval e2 env1 cont
      )
  ...
  and eq_val v1 v2 =
    let equal_lists l1 l2 =
      ...
      List.for_all2 (fun v1 v2 -> match eq_val v1 v2 with
        | Ok b -> b
        | Error _ -> false) l1 l2
    in
    match (v1, v2) with
  ...
  and app funpart arg cont =
    match funpart with
    | FunVal (x, body, env1) ->
        eval body (Environment.ext env1 x arg) cont
